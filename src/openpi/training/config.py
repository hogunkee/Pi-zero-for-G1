...
class LeRobotG1DataConfig(DataConfigFactory):
    ...
    #             _transforms.RepackTransform(
    #                 {
    #                     "images": {"cam_high": "observation.images.rs_view"},
    #                     "state": "observation.state",
    #                     "actions": "action",
    #                 }
    #             )
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        # "images": {"cam_high": "observation.images.rs_view"},
                        "observation/image": "observation.images.rs_view",
                        "state": "observation.state",
                        "actions": "action",
                        "prompt": "prompt",
                    }
        ...
        if self.use_delta_joint_actions:
            delta_action_mask = _transforms.make_bool_mask(7, -7, 7, -7, 3)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        # Here, we use LeRobot data format (like for all other fine-tuning examples)
        # To convert your custom G1 dataset (<10s of hours) to LeRobot format, see examples/droid/convert_droid_data_to_lerobot.py
        name="pi0_g1_finetune",
        model=pi0_config.Pi0Config(action_horizon=16),
        data=LeRobotG1DataConfig(
            # Replace with your custom G1 LeRobot dataset repo id.
            # repo_id="nvidia/PhysicalAI-Robotics-GR00T-Teleop-G1", #"hogunkee/g1_dataset",
            repo_id="/data1/hogun/dataset/G1_PnPCanToPlate_Dataset",
            # repo_id="/data1/hogun/dataset/G1_Collecting_PnPCupToPlate",
            # repo_id="local",
            # dataset_path=["/data1/hogun/dataset/G1_Collecting_PnPCupToPlate"],
            base_config=DataConfig(prompt_from_task=True),
            assets=AssetsConfig(
                # Important: reuse the original G1 norm stats during fine-tuning!
                assets_dir="/data1/hogun/dataset/G1_PnPCanToPlate_Dataset",
                # assets_dir="/data1/hogun/dataset/G1_Collecting_PnPCupToPlate",
                # assets_dir="gs://openpi-assets/checkpoints/pi0_base/assets",
                asset_id="g1",
            ),
        ),
    ...
    # g1 lora fine-tuning config
    TrainConfig(
        name="pi0_g1_lora_finetune",
        model=pi0_config.Pi0Config(action_horizon=16, paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotG1DataConfig(
            repo_id="/data1/hogun/dataset/G1_PnPCanToPlate_Dataset",
            # repo_id="/data1/hogun/dataset/G1_Collecting_PnPCupToPlate",
            base_config=DataConfig(prompt_from_task=True),
    ...
    TrainConfig(
        name="pi0_fast_g1_finetune",
        model=pi0_fast.Pi0FASTConfig(
            action_dim=31, action_horizon=16, max_token_len=250, paligemma_variant="gemma_2b_lora"
        ),
        data=LeRobotG1DataConfig(
            repo_id="/data1/hogun/dataset/G1_Collecting_PnPCupToPlate",
            base_config=DataConfig(prompt_from_task=True),
    ...
    # g1 fast lora fine-tuning
    TrainConfig(
        name="pi0_fast_g1_lora_finetune",
        # Here is an example of loading a pi0-FAST model for LoRA finetuning.
        # For setting action_dim, action_horizon, and max_token_len, see the comments above.
        model=pi0_fast.Pi0FASTConfig(
            action_dim=31, action_horizon=16, max_token_len=250, paligemma_variant="gemma_2b_lora"
